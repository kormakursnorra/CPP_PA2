...
ARRAY_TEST_BIN := $(BIN_DIR)/$(ARRAY_TEST_TARGET)

ALL_TARGETS := $(DLL_TEST_BIN) $(HEAP_TEST_BIN) $(HASHMAP_TEST_BIN) $(BST_TEST_BIN) $(DEQUE_TEST_BIN) $(ARRAY_TEST_BIN)
...
ARRAY_TEST_SOURCES := src/Array/array.cpp
...
ARRAY_TEST_OBJECTS := $(ARRAY_TEST_SOURCES:$(SRC_DIR)/%.cpp=$(OBJ_DIR)/%.o)
...
$(ARRAY_TEST_BIN): $(ARRAY_TEST_OBJECTS) | $(BIN_DIR)
	$(CXX) $(CXXFLAGS) $^ -o $@ $(LDFLAGS)
	@chmod +x $@
	@echo "Build Array complete: $@"
...
array_test: $(ARRAY_TEST_BIN)

.PHONY: all run-dll run-heap run-hashmap run-deque run-array test clean rebuild list help dll_test heap_test hashmap_test bst_test deque_test array_test
